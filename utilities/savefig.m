...
%	device.png.cmyk=	sprintf(cmdEnd,	'png???', 					[fname '.png']);
	...
	switch(computer)													% Get gs command.
		case {'MAC','MACI'},			gs= '/usr/local/bin/gs'; fileTmp = '/savefig-temp.eps';
		case {'PCWIN','PCWIN64'},		gs= 'gswin32c.exe'; fileTmp = '/savefig-temp.eps';
		otherwise,						gs= 'gs'; fileTmp = '~/savefig-temp.eps';
	end
...
% 	print(fighdl, deps{:}, '-noui', renderer, res, [fname '-temp.eps']);	% Output the eps.
    print(fighdl, deps{:}, '-noui', renderer, res, fileTmp);	% Output the eps.
	...
	for n= 1:length(types)												% Output them.
		...
		cmd=	sprintf('%s %s %s -f "%s"', gs, cmd, cmp, fileTmp);% Add up.
			status= system(cmd);										% Run Ghostscript.
			if (op_dbg || status), 		display (cmd),		end
	end
	recycle(fileTmp);										% Clean up.
end
